...
// Functor so we can use std::accumulate on phi(x_i)
/*
struct phiAcc
{
  double operator() ( double result, const Particle & p )
  {
	std::cout << "result = " <<result << std::endl;
	std::cout << "&p = " << &p << std::endl;
	std::cout << "p.getMass() = " <<p.getMass() << std::endl;
	std::cout << "phi(p.getMass()) = " <<phi(p.getMass()) << std::endl;

	
	return result + phi(p.getMass());
  }
};
*/
  ...
  for(int run = 0; run < L; run++)
    {
      ...
      for (vector<Particle>::iterator iterPart = mfaParticle.begin(); iterPart != mfaParticle.end(); ++iterPart)
		{
		  ...
		  //iterPart->setMass(-2.0+(iterPart-mfaParticle.begin())*4.0e0/(N-1.0e0)); // linear
	  ...
	  //updateTree(tree, height, mfaParticle);
	  growTree(tree, height, mfaParticle);
	  ...
	  */
	  ...
	  for (int ind = 0; ind < 10000; ++ind)
		{
		  double u = uniformRealDist(generator);
		  double y = u*phiSum;
		  
		  int partIndex = generateIndex(tree, height, y);//u*phiSum);
		  std::cout << "u = " << u << std::endl;
		  std::cout << "y = " << y << std::endl;
		  std::cout << "partIndex = " << partIndex << std::endl;
		  //cout << "particle mass  = " <<  mfaParticle[partIndex].getMass()  << endl;
		  std::cin.sync();
		  std::cin.get();
		  
		  
		  
		indicesDump << mfaParticle[partIndex].getMass() << endl;
	  ...
	  */
	  ...
	  /*
	  std::cout << "collRate = " << collRate << std::endl;
	  cout << "tree[0]  = " <<  tree[0]  << endl;
	  std::cin.sync();
	  std::cin.get();
	  */
	  ...
	  if(uniformRealDist(generator)<rate[0]/rateTotal)
	    {
		  ...
		  if(debug)
			{
			  cout << "firstParticle = " << firstParticle << endl;
			  cout << "secondParticle = " << secondParticle << endl;
			  
			  cout << "Before coag ";
			  printMasses(mfaParticle);
			  
			  // Previous contribution to distribution
			  cout << "Previous contribution to distribution, phi(mfaParticle[firstParticle].getMass()) = "
				   << phi(mfaParticle[firstParticle].getMass()) << endl;
			}
		  

		  // Coagulate particles and calculate contribution to binary tree
		  double deltaMass = -phi(mfaParticle[firstParticle].getMass());
		  ...
		  if(debug)
			{
			  ...
			  cout << "New contribution to distribution, phi(mfaParticle[firstParticle].getMass()) = "
				   << phi(mfaParticle[firstParticle].getMass()) << endl;
			}
		  ...
		  if(debug)
			{
			  ...
			  // Let's build a new tree and see how it differs from the updated tree
			  vector<double> newTree(pow(2,height)); // root node is now tree[1] and child sum is stored in tree[0]
			  growTree(newTree, height, mfaParticle);
		  
			  //cout << "New tree" << endl;
			  //printTree(newTree, height);
			  double maxDiff = 0;
			  
			  for (int ind = 0; ind < tree.size(); ++ind)
				{
				  double diff = abs(newTree[ind] - tree[ind]);
				  if (diff>maxDiff)
					maxDiff = diff;
				  
				  if(diff > 0)
					{
					  cout << "Tree differences, diff[" << ind << "] = " << newTree[ind] - tree[ind] << endl;
			  ...
			  double phiSum = std::accumulate(mfaParticle.begin(),mfaParticle.end(),0.0,[](double s, const Particle & p){return s + phi(p.getMass());});
			  // Dump: t firstParticle mfaParticle[firstParticle].getMass() phiSum tree[0] deltaMass
			  indicesDump.precision(10);
			  indicesDump.width(20);
			  indicesDump.setf(ios::scientific);
			  
			  indicesDump << t << "\t\t"
						  << maxDiff << "\t\t"
						  << firstParticle << "\t\t"
						  << mfaParticle[firstParticle].getMass() << "\t\t"
						  << phiSum << "\t\t"
						  << tree[0] << "\t\t"
						  << deltaMass << "\t\t"
						  << moments[2]
